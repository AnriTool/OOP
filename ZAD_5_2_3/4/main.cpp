...
виде массива.
Шаблон должен включать в себя функции добавления данных, удаления
данных, вывода данных на консоль ( info() ). При этом добавление данных должно осуществляться так,
чтобы каждый элемент встречался не более двух раз (то есть не
добавлять данное, если оно уже дважды есть в массиве).
Предусмотреть обработку следующих исключений:
добавление данных в заполненный массив, удаление данных из
...
int main()
{
    ...
    {
        std::cout << "Contaner with two equal elements!" << "\n";
    }
